<template>
    ...
    <ul>
      <li v-for="item in cartDetails" :key="item.id" class="cart-item">
        <img :src="imageUrls[item.id]" alt="Product Image" class="cart-item-image" />
  ...
  // API để lấy thông tin chi tiết sản phẩm
  const apiBaseUrl = 'https://67414054e4647499008d305b.mockapi.io/api/v1/products/';
//   const cartDetails = ref([]);
  ...
  async function fetchProductDetails() {
    ...
    for (const productId in cart) {
      ...
      productDetails.push({
        ...
        name: product.name,
        price: product.price,
        quantity: cart[productId], // Lấy số lượng từ localStorage
        imageUrl: imageUrls[product.id]  // Thêm ảnh sản phẩm
      });
    }
    cartDetails.value = productDetails;
  }
  ...
  function formatPrice(price) {
    ...
    return formattedPrice.replace('₫', '000').trim() + ' ₫';
  }
